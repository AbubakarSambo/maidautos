    ...
    <div className="space-y-4">
      ...
      {(showForm || editing) && (
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-xl border p-4 space-y-3">
          <h3 className="font-semibold text-gray-900">{editing ? 'Edit Stop' : 'Add New Stop'}</h3>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-gray-500">City / Terminal Name</label>
              <input {...register('name', { required: true })} placeholder="Abuja" className="mt-0.5 w-full px-3 py-2 border rounded-lg text-sm" />
            </div>
            <div>
              <label className="text-xs text-gray-500">State</label>
              <input {...register('state', { required: true })} placeholder="FCT" className="mt-0.5 w-full px-3 py-2 border rounded-lg text-sm" />
            </div>
          </div>
          <div className="flex gap-2">
            <button type="submit" disabled={creating || updating} className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-green-700 disabled:opacity-50">
              Save
            </button>
            <button type="button" onClick={() => { setShowForm(false); setEditing(null) }} className="px-4 py-2 rounded-lg text-sm border hover:bg-gray-50">
              Cancel
            </button>
          </div>
        </form>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {stops.map((stop) => (
            <div key={stop.id} className="bg-white rounded-xl border p-4 flex items-center gap-4">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => startEdit(stop)} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg">
