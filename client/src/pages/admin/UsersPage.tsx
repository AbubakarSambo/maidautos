...
export function AdminUsersPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <form onSubmit={handleSubmit((d) => create({ ...d, role: 'ADMIN' }))} className="bg-white rounded-xl border p-4 space-y-3">
          <h3 className="font-semibold text-gray-900">Add City Admin</h3>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="text-xs text-gray-500">Email</label><input {...register('email', { required: true })} type="email" className="mt-0.5 w-full px-3 py-2 border rounded-lg text-sm" /></div>
            <div><label className="text-xs text-gray-500">Phone</label><input {...register('phone')} className="mt-0.5 w-full px-3 py-2 border rounded-lg text-sm" /></div>
            <div><label className="text-xs text-gray-500">City</label><input {...register('adminCity')} placeholder="Abuja" className="mt-0.5 w-full px-3 py-2 border rounded-lg text-sm" /></div>
            <div><label className="text-xs text-gray-500">Password</label><input {...register('password', { required: true, minLength: 8 })} type="password" className="mt-0.5 w-full px-3 py-2 border rounded-lg text-sm" /></div>
          </div>
          <div className="flex gap-2">
            <button type="submit" disabled={isPending} className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-green-700 disabled:opacity-50">Create Admin</button>
      ...
      <div className="space-y-2">
        {isLoading ? <div className="py-8 text-center text-gray-500">Loading...</div> : users.map((user: any) => (
          <div key={user.id} className={`bg-white rounded-xl border p-4 flex items-center gap-3 ${!user.isActive ? 'opacity-50' : ''}`}>
            ...
            <div className="flex items-center gap-2">
              <span className={`text-xs px-2 py-0.5 rounded-full ${user.role === 'SUPER_ADMIN' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>{user.role}</span>
