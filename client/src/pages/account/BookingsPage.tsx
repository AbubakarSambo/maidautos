...
import { Bus, ArrowRight, Calendar, UserCircle, Printer } from 'lucide-react'
  ...
  return (
    <div className={`bg-white rounded-2xl border border-gray-100 shadow-sm p-4 hover:border-primary/40 hover:shadow-md transition-all ${faded ? 'opacity-60' : ''}`}>
      ...
      <div className="mt-3 flex gap-2">
        <button
          onClick={(e) => { e.stopPropagation(); window.open(`/booking/confirmation/${booking.ticketCode}?print=1`, '_blank') }}
          className="flex items-center justify-center gap-1.5 text-xs font-semibold text-gray-600 hover:bg-gray-50 py-2 px-3 rounded-lg border border-gray-200 transition-colors"
        >
        ...
        {onCancel && (
          <button
            ...
            className="flex-1 text-center text-xs font-semibold text-red-600 hover:bg-red-50 disabled:opacity-50 py-2 rounded-lg border border-red-200 transition-colors"
          >
