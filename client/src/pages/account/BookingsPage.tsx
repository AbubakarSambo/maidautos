    ...
    <div className="max-w-lg mx-auto p-4 space-y-6">
      ...
      {bookings.length === 0 && (
        <div className="text-center py-16">
          ...
          <button onClick={() => navigate('/')} className="mt-4 bg-green-600 text-white px-6 py-2 rounded-lg text-sm font-semibold">Book a ride</button>
        </div>
      )}
    </div>
  )
...
function BookingCard({ booking, onClick, faded }: { booking: Booking; onClick: () => void; faded?: boolean }) {
  return (
    <div onClick={onClick} className={`bg-white rounded-xl border p-4 cursor-pointer hover:border-green-300 transition-all ${faded ? 'opacity-60' : ''}`}>
      <div className="flex items-center gap-2 mb-2">
        <span className="font-semibold text-gray-900">{booking.pickupStop.stop.name}</span>
        <ArrowRight className="w-4 h-4 text-gray-400" />
        <span className="font-semibold text-gray-900">{booking.dropoffStop.stop.name}</span>
      ...
      <div className="mt-2 flex gap-2">
        <span className={`text-xs px-2 py-0.5 rounded-full ${booking.status === 'CONFIRMED' ? 'bg-green-100 text-green-700' : booking.status === 'CANCELLED' ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600'}`}>
          {booking.status}
        </span>
        <span className={`text-xs px-2 py-0.5 rounded-full ${booking.paymentStatus === 'PAID' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'}`}>
